...
import com.psl.ibm.spec.constraints.AtLeastOneContact;
import com.psl.ibm.spec.constraints.HasId;
...
public class EmployeeResource {
	...
	public Response getEmployees(@QueryParam(Constant.QUERY_PARAM_ORDERBYASC) String orderByAsc,
			...
			@QueryParam(Constant.QUERY_PARAM_NAME) String name, @QueryParam(Constant.QUERY_PARAM_ID) String id)
			throws ValidationException, EmployeeException {

		employeeFetchParametersValidation(orderByAsc, orderByDesc);

		Employees employees = new EmployeeDAOImpl().getEmployees(orderByAsc, orderByDesc, designation, name, id, null);

		for (Employee employee : employees.getEmployees()) {
			UriBuilder selfie = uriInfo.getBaseUriBuilder();
			selfie.path(EmployeeResource.class, "getEmployeeById");
			employee.setSelf(new AtomLink(selfie.build("employees/" + employee.getEmpId()).toString(),
	...
	@Path("{empid}")
	@Produces(value = { MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response getEmployeeById(@PathParam(Constant.EMPLOYEE_ID) String empid)
		...
		Employees employees = new EmployeeDAOImpl().getEmployees(null, null, null, null, null,
	...
	@HasId
	public Response createEmp(@NotNull @AtLeastOneContact(message = "{contact.empty.means}") @Valid Employee employee)
			throws ValidationException, EmployeeException {
		List<String> errorMessages = new ArrayList<String>();
	...
	public Response updateEmp(@PathParam(Constant.EMPLOYEE_ID) String empid,
			@FormParam(Constant.FORM_PARAM_NAME) String empName, @FormParam(Constant.FORM_PARAM_AGE) int empAge,
			@FormParam(Constant.FORM_PARAM_DESIGNATION) String desig) throws ValidationException, EmployeeException {
		...
		return Response.status(Status.OK).entity("Employee Updated: " + employee).build();
	}
	...
	@DELETE
	@Path("{empid}")
	@Produces(value = { MediaType.TEXT_HTML, MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response deleteEmp(@PathParam(Constant.EMPLOYEE_ID) String empid) throws EmployeeException {
	...
	private void employeeFetchParametersValidation(String orderByAsc, String orderByDesc) throws EmployeeException {
		List<String> errorMessages = new ArrayList<String>();
		if (!(orderByAsc == null || Constant.SORT_BY_NAME.equalsIgnoreCase(orderByAsc)
				|| Constant.SORT_BY_AGE.equalsIgnoreCase(orderByAsc)
				|| Constant.SORT_BY_ID.equalsIgnoreCase(orderByAsc))) {
			errorMessages
					.add("Sorting in ascending order can only be done on age, name or id! \n Check your keywords \n");
		}
		if (!(orderByDesc == null || Constant.SORT_BY_NAME.equalsIgnoreCase(orderByDesc)
				|| Constant.SORT_BY_AGE.equalsIgnoreCase(orderByDesc)
				|| Constant.SORT_BY_ID.equalsIgnoreCase(orderByDesc))) {
			errorMessages
	 ...
	 * @throws EmployeeException
	 */
	private void employeeCreateValidation(String empName, int empAge, Designation designation)
			throws EmployeeException {
