...
public class IbmsDAOImpl implements IbmsDAO {
	...
	public Request getRequest(int id) {
		...
		Session session = new HibernateSessionConnection().getSession();
		Transaction tx = session.beginTransaction();
		Request request = (Request) session.get(Request.class, new Integer(id));
		tx.commit();
		session.close();
		return request;
	}
	...
	public void processRequest(Request request) {
		...
		for (BloodBank u : bb) {
			// System.out.println(u.getBloodBank_address()+"\t\t\t\t\t\t"
		...
		if (list_yes.size() == 0 && list_yesAll.size() == 0) {
			...
			try {
				sms.sendMessage("+91"
						+ request.getBloodBank().getBloodBank_mobileNo()
								.toString(), message);
				sms.sendMessage("+91"
						+ request.getHospital().getHospital_contactNo()
								.toString(), message2);
		...
		if (map_yesAll.size() != 0) {
			...
			obj.addRequest(request);
			SmsSender sms = new SmsSender();
			String message = "You will receive sample from hospital for requestId:"
					+ request.getRequestId();
			String message2 = "XYZ will come before 5PM to collect blood sample for requestId:+"
					+ request.getRequestId();
			try {
				sms.sendMessage(
						"+91" + key1.getBloodBank_mobileNo().toString(),
						message);
				sms.sendMessage("+91"
						+ request.getHospital().getHospital_contactNo()
								.toString(), message2);
		...
		} else if (map_yes.size() != 0) {
			BloodBank key1 = (BloodBank) map_yes.keySet().toArray()[0];
			request.setBloodBank(key1);

			/*
			 * url = key1.getBloodBank_rest_url(); url = url + "/getQuantity";
			 * WebResource resource = client.resource(url); Form form = new
			 * Form(); form.add("bloodGroup", request.getBloodgroup());
			 * form.add("bloodType", request.getBloodType());
			 * 
			 * ClientResponse res = resource.type(
			 * MediaType.APPLICATION_FORM_URLENCODED_TYPE).post(
			 * ClientResponse.class, form); String msg =
			 * res.getEntity(String.class); int quantity_received =
			...
			String message = "You will receive sample from hospital for requestId:"
					+ request.getRequestId()
					+ " Also contact donors for the same.";
			String message2 = "XYZ will come before 5PM to collect blood sample for requestId:+"
					+ request.getRequestId();
			try {
				sms.sendMessage(
	...
	public void acknowledgeRequest(Request request) {
		...
		obj.addRequest(request);
		SmsSender sms = new SmsSender();
		String message = "A person will be coming to collect the blood for requestId:"
				+ request.getRequestId();
		String message2 = "You will receive the required blood by 6PM today";
		
		int getQnty=Integer.parseInt((new BloodBankDAOImpl().getUnits(
							request.getBloodBank(), request.getBloodgroup(),
							request.getBloodType())));
		if(getQnty>=request.getQuantity_required()){
			request.setQuantity_received(request.getQuantity_required());
			System.out.println("\n\n\n\n\n\n"+request.getQuantity_received());
			new HospitalDAOImpl().addRequest(request);
		}else{
			request.setQuantity_received(Integer
					.parseInt((new BloodBankDAOImpl().getUnits(
							request.getBloodBank(), request.getBloodgroup(),
							request.getBloodType()))));
			System.out.println("\n\n\n\n\n\n"+request.getQuantity_received());
			new HospitalDAOImpl().addRequest(request);
			...
			sms.sendMessage(
					"+91"
							+ request.getBloodBank().getBloodBank_mobileNo()
									.toString(), message);
			sms.sendMessage("+91"
					+ request.getHospital().getHospital_contactNo().toString(),
					message2);
	...
	public void denyRequest(Request request) {
		...
		/*
		 * if (map_yesAll.containsKey(b)) { map_yesAll.remove(b); if
		 * (map_yesAll.size() >= 0) { BloodBank key1 = (BloodBank)
		 * map_yesAll.keySet().toArray()[0]; request.setBloodBank(key1);
		 * HospitalDAO obj=new HospitalDAOImpl(); obj.addRequest(request); }
		 * else { if (map_yes.size() >= 0) { BloodBank key1 = (BloodBank)
		 * map_yes.keySet().toArray()[0]; request.setBloodBank(key1);
		 * HospitalDAO obj=new HospitalDAOImpl(); obj.addRequest(request); //
		 * check stock } else { contactDonors(b, request); } } } else if
		 * (map_yes.containsKey(b)) { map_yes.remove(b); if (map_yes.size() >=
		 * 0) { BloodBank key1 = (BloodBank) map_yes.keySet().toArray()[0];
		 * request.setBloodBank(key1); HospitalDAO obj=new HospitalDAOImpl();
		 * obj.addRequest(request); // check stock } else { contactDonors(b,
		 * request); } }
		 */
